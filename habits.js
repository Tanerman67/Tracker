...
function renderHabitEditor(){
  ...
  let content=duration+`<details class="habit-extra"><summary>${habitLabel('Подробности — необязательно','Details — optional')}</summary>`;
  if(mode==='sleep'){
...
function editHabitActivity(index){
  ...
  if(key==='blocks')document.getElementById('habitActivityKind').value=item.kind;
...
function addHabitActivity(){
  ...
  const key=habitEditor.mode==='outdoors'?'walks':'blocks',kind=key==='blocks'?document.getElementById('habitActivityKind').value:null;
  ...
  if(key==='walks')item.purpose=previous?.purpose||'walk';else item.kind=kind==='break'?'break':'focus';
...
function renderHabitWeek(){
  ...
  host.innerHTML=`<div class="habit-week-rows">${rows.map(([title,text])=>`<div><strong>${escapeHTML(title)}</strong><p>${escapeHTML(text)}</p></div>`).join('')}</div>`;
}

function renderSimpleProgress(){
  const w=weeklyWindow(),s=habitWeekData(w.currentKeys),n=todayNumber;
  document.getElementById('simpleProgressTitle').textContent=trText('Эта неделя','This week');
  document.getElementById('simpleProgressPeriod').textContent=weeklyPeriodLabel(w.start,w.end);
  document.getElementById('simpleProgressDetails').textContent=trText('Подробности недели','Week details');
  const rows=[];
  if(s.durations.length)rows.push(trText(
    `Сон: в среднем ${n(s.durations.reduce((a,b)=>a+b,0)/s.durations.length)} ч · записей: ${s.durations.length}/${s.days} дней.`,
    `Sleep: average ${n(s.durations.reduce((a,b)=>a+b,0)/s.durations.length)} h · ${s.durations.length}/${s.days} days logged.`));
  if(s.steps.length)rows.push(trText(
    `Шаги: в среднем ${n(s.steps.reduce((a,b)=>a+b,0)/s.steps.length)} · записей: ${s.steps.length}/${s.days} дней.`,
    `Steps: average ${n(s.steps.reduce((a,b)=>a+b,0)/s.steps.length)} · ${s.steps.length}/${s.days} days logged.`));
  if(s.outdoorsTotal!==null)rows.push(trText(
    `На улице: ${n(s.outdoorsTotal)} мин · записей: ${s.outdoorTotals.length}/${s.days} дней.`,
    `Outdoors: ${n(s.outdoorsTotal)} min · ${s.outdoorTotals.length}/${s.days} days logged.`));
  if(rows.length<3&&s.workoutLogged)rows.push(trText(`Тренировок записано: ${s.workoutDays}.`,`Workouts logged: ${s.workoutDays}.`));
  document.getElementById('simpleProgressRows').innerHTML=rows.length
    ?rows.slice(0,3).map(x=>`<p>${escapeHTML(x)}</p>`).join('')
    :`<p>${habitLabel('Пока недостаточно записей для выводов за неделю.','Not enough records for weekly observations yet.')}</p>`;
  renderHabitWeek();
}
